# Authors and Citation

## Authors

- **Nan Xiao**. Author, maintainer, copyright holder.
  [](https://orcid.org/0000-0002-0250-5673)

- **Joshua Cook**. Contributor.

- **Clara Jégousse**. Contributor.

- **Hui Chen**. Contributor.

- **Miaozhu Li**. Contributor.

- **iTerm2-Color-Schemes contributors**. Contributor, copyright
  holder.  
  iTerm2-Color-Schemes project

- **Winston Chang**. Contributor, copyright holder.  
  staticimports.R

## Citation

Source:
[`DESCRIPTION`](https://github.com/nanxstats/ggsci/blob/v4.3.0/DESCRIPTION)

Xiao N (2026). *ggsci: Scientific Journal and Sci-Fi Themed Color
Palettes for 'ggplot2'*. R package version 4.3.0,
<https://nanx.me/ggsci/>.

    @Manual{,
      title = {ggsci: Scientific Journal and Sci-Fi Themed Color Palettes for 'ggplot2'},
      author = {Nan Xiao},
      year = {2026},
      note = {R package version 4.3.0},
      url = {https://nanx.me/ggsci/},
    }
